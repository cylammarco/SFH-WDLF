...
Mbol_err = np.load("SFH-WDLF-article/figure_data/Mbol_error_recomputed.npy", allow_pickle=True).item()
...
mag_mean, err_mean = np.load("SFH-WDLF-article/figure_data/mbol_err_mean.npy").T
...
ax3.plot(age, mag_resolution_itp(age), color="black", ls="dashed", label="Fitted")
ax3.grid()
ax3.legend()
...
ax5.scatter(mag_bin_mid, resolution_optimal, s=5)
...
plt.subplots_adjust(top=0.99, bottom=0.045, left=0.1, right=0.985, hspace=0.015)

plt.savefig(os.path.join(figure_folder, "fig_03_mbol_sigma_magnitude_resoltuion.png"))
