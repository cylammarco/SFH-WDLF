...
def eq2gal(ra, dec, intype="degrees", outtype="degrees"):
    ...
    try:
        ind = np.where(lgal < 0.0)
        lgal[ind] = lgal[ind] + 2.0 * np.pi
    except:
        if lgal < 0.0:
            lgal = lgal + 2.0 * np.pi
    if outtype == "degrees":
...
def find_d_limit(max_dist, parallax, mag, par_itp, sig, mag_low, mag_high):
    ...
    if max_dist > 100.0:
        return np.inf
    mod = np.log10(1000.0 / parallax) * 5.0 - 5.0
    max_mod = np.log10(max_dist) * 5.0 - 5.0
...
for i in range(hp.nside2npix(2)):
    par_err_spl[i] = pickle.load(
        open(
            "/Users/marcolam/Desktop/Gaia_phot/nside_2/par_err_pix_"
            + str(i)
            + ".pkl",
            "rb",
        )
...
pix = hp.ang2pix(2, np.radians(90.0 - data["dec"]), np.radians(data["ra"]))
...
for i in range(len(data)):
    ...
    for j in range(len(par_err_spl)):
        ...
        vol[i, j] = (
            volume(1000.0 / data["parallax"][i], sinb[i], 20.0, 250.0)
            * 4.0
            * np.pi
        )
        vmax[i, j] = volume(dmax[i, j], sinb[i], 20.0, 250.0) * 4.0 * np.pi
...
num, mag = np.histogram(
    data["phot_g_mean_mag"] - np.log10(1000.0 / data["parallax"]) * 5.0 + 5,
    weights=48.0 * 4.0 / np.sum(vmax, axis=1),
    bins=60,
    range=(5, 20),
)
mag = mag[1:] + 0.25
