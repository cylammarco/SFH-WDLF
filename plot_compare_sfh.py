...
rowell_age, rowell_sfh, _ = np.genfromtxt(
    "test_case/fig_17.txt", delimiter=" "
).T
rowell_age /= 1.0e9
...
_rowell_steps = plt.step(
    rowell_age, rowell_sfh, where="mid", label="Rowell 2013"
)
_mor_steps = plt.step(mor_age, mor_sfh, where="mid", label="Mor et al. 2019")
_isern_s_steps = plt.step(
    isern_age_s, isern_sfh_s, where="mid", label="Isern 2019"
)
_isern_ns_steps = plt.step(
    isern_age_ns, isern_sfh_ns, where="mid", label="Isern 2019 (NS)"
)

lam_age_width = np.diff(
    np.append(
        _lam_steps[0]._path._vertices[:, 0][::2],
        _lam_steps[0]._path._vertices[:, 0][-1],
    )
)
rowell_age_width = np.diff(
    np.append(
        _rowell_steps[0]._path._vertices[:, 0][::2],
        _rowell_steps[0]._path._vertices[:, 0][-1],
    )
)
mor_age_width = np.diff(
    np.append(
        _mor_steps[0]._path._vertices[:, 0][::2],
        _mor_steps[0]._path._vertices[:, 0][-1],
    )
)
isern_s_age_width = np.diff(
    np.append(
        _isern_s_steps[0]._path._vertices[:, 0][::2],
        _isern_s_steps[0]._path._vertices[:, 0][-1],
    )
)
isern_ns_age_width = np.diff(
    np.append(
        _isern_ns_steps[0]._path._vertices[:, 0][::2],
        _isern_ns_steps[0]._path._vertices[:, 0][-1],
    )
)
