...
\title[Galactic SFH from Gaia EDR3 WDLFs]{The Galactic Star Formation History from the Gaia EDR3 White Dwarf Luminosity Functions}
...
(Look up the 25/40/70/100 pc samples)
...
(Check the Canadian work...)


\subsection{Forward modelling of WDLFs}

Wood (1990), Noh (some year), and all the WDLF-related work as previouslymetioned have all compared their observed WDLFs to the theoretical counterparts. However, due to the lack of resolution in the WDLFs in the pre-Gaia era, there is little statistical leberage to match the observed and theoretical WDLFs that can only give previse but uncertain solution. These previous work only ``fit'' the WDLFs to a prevision of 1 Gyr and with few and simple SFH scenario.
...
When fitting data to model, there is always an associated uncertainty in the fitted result. By modelling the uncertainty as a function of the true M$_{\mathrm{bol}}$, we can treat it as the ``smoothing kernel'' that applied onto the true SFH with infinite resolution. Thus, it gives us the limiting resolution that allows the choice of an optimal binning of the
data such that we can use all the features in the WDLFs as they can be ruled out as random noise.
...
The limiting resolution is when the feature variation is at the resolution of section 4.3.
